...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        ...
        h, w, _ = im.shape
        if aspect_ratio > 1.0:
            im = np.array(Image.fromarray(im).resize((h, int(w * aspect_ratio)), resample='bicubic'))
        if aspect_ratio < 1.0:
            im = np.array(Image.fromarray(im).resize((int(h / aspect_ratio), w), resample='bicubic'))
