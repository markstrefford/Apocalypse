...
class UnalignedSegDataset(BaseDataset):
	...
	def initialize(self, opt):
		...
		self.max_instances = 10  # default: 20
	...
	def read_segs(self, seg_path, seed):
		...
		for i in range(self.max_instances):
			...
			if os.path.isfile(path):
				seg = Image.open(path).convert('L').convert('RGB')  # Forcing to 3-channel as that's what transform needs
				seg = self.fixed_transform(seg, seed)
				segs.append(seg)
			else:
				if i == 0:    # Identifying images where there's no mask! Helps clean dataset
					print('No mask file {} found!!'.format(path))
	...
	def __getitem__(self, index):
		...
		# print('__getitem__(): A_path={}'.format(A_path))
		# print('__getitem__(): B_path={}'.format(B_path))
		...
		# print('__getitem__(): A_seg_path={}'.format(A_seg_path))
		# print('__getitem__(): B_seg_path={}'.format(B_seg_path))
